...
def caculateDistance(startPoint, endPoint):  # 计算欧氏距离
    return sum((startPoint-endPoint)**2)**0.5


def iriskNN_Classify(dataset, input, decideNumber, specialPoint):  # 改进kNN分类
    length = len(dataset)
    distances = []
    labels = {}
...
def traditional_iriskNN_Classify(dataset, input, decideNumber):  # 传统kNN分类
    length = len(dataset)
    distances = []
    labels = {}
    for pointer in range(length):
...
def elder_check(dataset, decideNumber):
    labels = {}
    falseLabel = {}
    result = {}
    for data in dataset:
        ...
        labels[realLabel] += 1
        predictLabel = traditional_iriskNN_Classify(dataset, data[:-1], decideNumber)
        if predictLabel!=realLabel:
            falseLabel[realLabel] += 1
    for label in labels:
        total = labels[label]
        loss = falseLabel[label]
        result[label] = str(100 * (total-loss)/total)+"%"
    return result


def check(dataset, decideNumber, specialPoint):  # 检验
    labels = {}
    falseLabel = {}
    result = {}
    for data in dataset:
...
def iriskNN_specialPoint_detector(dataset, decideNumber, specialRange):  # 特殊点探测
    length = len(dataset)
    distances = []
    specialPoint = []
    labels = {}
...
prediction = traditional_iriskNN_Classify(dataset, [5.9, 3.2, 4.8, 1.8], 7)
print("It is " + prediction)
checkResult = elder_check(dataset, 7)
print(checkResult)
